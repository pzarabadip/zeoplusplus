...
voronoi_network::voronoi_network(c_class &c, double net_tol_)
    : bx(c.bx), bxy(c.bxy), by(c.by), bxz(c.bxz), byz(c.byz), bz(c.bz),
      nx(c.nx), ny(c.ny), nz(c.nz), nxyz(nx * ny * nz), xsp(nx / bx),
      ysp(ny / by), zsp(nz / bz), net_tol(net_tol_) {
  int l;
  ...
  for (l = 0; l < edmem; l++)
    raded[l] = new block[init_network_edge_memory];
  for (l = 0; l < edmem; l++)
...
void voronoi_network::store_network(vector<VOR_NODE> &nodes,
  ...
  for (l = 0; l < edc; l++) {
    ...
    vector<int> atomIDs;
    for (ll = 0; ll < nec[l]; ll++)
      atomIDs.push_back(ne[l][ll]);
  ...
  for (l = 0; l < c.p; l++, vmp += 4) {
    ...
    if (search_previous(gx, gy, vx, vy, vz, ijk, q, vmp[1], vmp[2], vmp[3])) {
      ...
      *vmp = idmem[ijk][q];
      ...
      if (edc == edmem)
        add_edge_network_memory();
      if (ptsc[ijk] == ptsmem[ijk])
        add_network_memory(ijk);
...
inline void voronoi_network::add_neighbor(int k, int idn) {
  for (int i = 0; i < nec[k]; i++)
    if (ne[k][i] == idn)
      return;
  if (nec[k] == numem[k])
    add_particular_vertex_memory(k);
  ne[k][nec[k]++] = idn;
}
