...
void FEATURE::segment_identify_connections() {
  ...
  vector<int> face_nodes_in_segment; // list IDs of nodes assigned to segment
                                     // and located on segment's "face", that is
    ...
    if (nodelist.size() == 1) {
             ...
             // node, merge with others
      {
        ...
        if (tobemerged[mink] > -1)
          mink = tobemerged[mink]; // if node to which we are going to merge the
                                   // current node also will be merged, merge
                                   // into the other node
...
void FEATURE::merge_newSegmentGrow(int segid) {
  ...
  for (k = 0; k < (int)segment_conn.size(); k++) {
    if ((segment_conn.at(k).from_seg == segid ||
      ...
      if (segment_merge_list.at(conn_to) == -1)
        merge_newSegmentGrow(conn_to);
  ...
  for (unsigned int i = 0; i < unitCells.size(); i++) {
    ...
    for (unsigned int j = 0; j < nodeIDs.size(); j++) {
      ...
      if (object.size() == 1) { // first sphere added
        ...
        if (min.x > (sph.x - sph.r))
          min.x = sph.x - sph.r;
        if (max.x < (sph.x + sph.r))
          max.x = sph.x + sph.r;
        if (min.y > (sph.y - sph.r))
          min.y = sph.y - sph.r;
        if (max.y < (sph.y + sph.r))
          max.y = sph.y + sph.r;
        if (min.z > (sph.z - sph.r))
          min.z = sph.z - sph.r;
        if (max.z < (sph.z + sph.r))
          max.z = sph.z + sph.r;
      };
    }
      ...
      for (int i = 0; i < (int)object.size(); i++) {
        ...
        if (object[i].r - r_tr > 0)
          for (int j = 0; j < N_MC_SA_SAMPLE; j++) {
            ...
            double xpoint = sin(phi) * cos(theta); // Don't need abs(sin(phi))
