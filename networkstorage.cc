...
ATOM_NETWORK::ATOM_NETWORK() {
  ...
  // matrix could be inverted; allowAdjustCoordsAndCellFlag is for the temporary
  // Voronoi volume error fix
...
void ATOM_NETWORK::randomlyAdjustCoordsAndCell() {
  ...
  printf("NOTICE: attempting random vector shift of all atom coordinates by %e "
  ...
  printf("NOTICE: randomly adjusted cell dimensions and angles: %e %e %e; %e "
         "%e %e\n",
  ...
  for (int i = 0; i < num_v; i++) {
    ...
    for (int j = 0; j < num_e; j++) {
      ...
      if (unit_edge_length < 0)
        ...
        printf("ERROR: found a basic edge length of %.3f which is sufficiently "
...
VOR_EDGE::VOR_EDGE(int myFrom, int myTo, double rad, int dx, int dy, int dz,
                   double len)
    : from(myFrom), to(myTo), rad_moving_sphere(rad), delta_uc_x(dx),
      delta_uc_y(dy), delta_uc_z(dz), length(len) {}
...
VORONOI_NETWORK::VORONOI_NETWORK(const XYZ &inp_va, const XYZ &inp_vb,
    ...
    : edges(inp_edges), nodes(inp_nodes), v_a(inp_va), v_b(inp_vb),
      v_c(inp_vc) {}

/// Copy constructor for VORONOI_NETWORK
VORONOI_NETWORK::VORONOI_NETWORK(const VORONOI_NETWORK &net)
    : v_a(net.v_a), v_b(net.v_b), v_c(net.v_c), edges(net.edges),
      nodes(net.nodes) {}
...
void pruneVoronoiNetworkfromEdgeList(VORONOI_NETWORK *vornet,
  ...
  while (edgeIter != vornet->edges.end()) {
    ...
    for (unsigned int i = 0; i < ids.size(); i++) {
      ...
      if (edgeIter->from == ids[i] || edgeIter->to == ids[i])
        flag++;
    };
      ...
      for (int i = 0; i < numAtoms; i++) {
        if (origNet.atoms[i].type.compare("Si") == 0 && atomSubs[i] == false) {
          ...
          if (!atomSubs[nlist[0]] && !atomSubs[nlist[1]] &&
              !atomSubs[nlist[2]] &&
              !atomSubs[nlist[3]]) // if all of neighbors are Si, position i can
                                   // be used
          {
...
int place_molecule(vector<MOLECULE> *molecules,
  ...
  if (num_placed == 0) {
    if (verbose)
      printf("DEBUG: detected that no molecules have yet been placed - "
    ...
    if (!placed->at(
      ...
      for (int j = 0; j < num_connections && connection_index == -1;
        ...
        if (!connected->at(j)) { // if this connection was already made, we
          ...
          if (c.v1 ==
              i) { // vertex 1 in this connection is the molecule we are placing
            if (placed->at(c.v2)) { // vertex 2 in this connection is a molecule
          ...
          } else if (c.v2 == i) { // vertex 2 in this connection is the molecule
                                  // we are placing
            if (placed->at(c.v1)) { // vertex 1 in this connection is a molecule
                                    // which has already been placed
          ...
          if (fixed_edge_index != -1 && placing_edge_index != -1 &&
              ...
              placing_vertex_index !=
                  -1) { // this connection connects this molecule and a fixed
                        ...
                        // molecule!
            if (verbose)
              printf("DEBUG: found that new vertex %d edge %d is connected to "
                     "fixed vertex %d edge %d\n",
                     placing_vertex_index, placing_edge_index,
                     fixed_vertex_index, fixed_edge_index);
            ...
            if (placing.atoms_type.at(placing_atom_index) !=
              ...
              if (verbose)
                printf("DEBUG: trying to connect conflicting connection sites "
            ...
            if (verbose)
              printf("DEBUG: site index in fixed = %d, site_index in placing = "
                     "%d\n",
                     fixed_site_index, placing_site_index);
            if (verbose)
              printf("DEBUG: fixed_site=%.3f %.3f %.3f; placing_site=%.3f %.3f "
                     "%.3f; shift=%.3f %.3f %.3f\n",
            ...
            double com_dist =
                (fixed.com - framework->at(placing_vertex_index).com)
                    .magnitude();
            if (verbose)
              printf("DEBUG: made a connection indicating a topological edge "
                     "length of %.3f\n",
                     com_dist);
            ...
            if (verbose)
              printf("\tDEBUG: this edge in the underlying net goes from "
                     "fractional %.3f %.3f %.3f to fractional %.3f %.3f %.3f\n",
                     start_abc.x, start_abc.y, start_abc.z, end_abc.x,
                     end_abc.y, end_abc.z);
            XYZ shift_abc = end_abc - start_abc;
            XYZ shift_xyz = framework->at(placing_vertex_index).com - fixed.com;
            if (verbose)
              printf("\tDEBUG: this edge in the underlying net had a "
                     "fractional shift %.3f %.3f %.3f; here we observe a "
      ...
      if (connection_index == -1) {
        if (verbose)
          printf("DEBUG: could not find any connection between vertex %d and "
                 "any fixed vertex\n",
                 i);
      }
    }
  }
  printf("WARNING: could not find a molecule which has not been placed and "
         "which is adjacent to an already placed molecule - this may happen if "
         "the net is interpenetrated (not currently handled with the "
         "connection-based alignment method)\n");
  return num_placed;
}
...
void create_unit_cell_from_vectors(vector<XYZ> *vecs, ATOM_NETWORK *cell) {
  ...
  for (int i = 0; i < num_vecs; i++) {
    if (!is_assigned.at(i)) { // found z
      if (found_z) {
        printf("ERROR: after setting x and y vectors, more than one vector "
  ...
  XYZ cross = x.cross(y);
  if (verbose)
    printf("DEBUG: x = %.3f %.3f %.3f, y = %.3f %.3f %.3f, cross = %.3f %.3f "
           "%.3f; need to know which orientation of z = %.3f %.3f %.3f "
           "satisfies right hand rule\n",
           x.x, x.y, x.z, y.x, y.y, y.z, cross.x, cross.y, cross.z, z.x, z.y,
           z.z);
  int z_scale = 1;
  if (cross.angle_between(z) > cross.angle_between(z.scale(-1)))
    z_scale = -1;
  z = z.scale(z_scale);
  if (verbose)
...
bool find_two_way_connections(ATOM_NETWORK *cell,
  ...
  for (int i = 0; i < num_v; i++) {
    ...
    for (int j = 0; j < num_e; j++) {
      ...
      for (int k = 0; k < num_v && overlap_v_index == -1; k++) {
        ...
        if (verbose)
          printf("DEBUG: checking for overlap between edge end position %.3f "
                 "%.3f %.3f and other vertex position %.3f %.3f %.3f\n",
                 e.x, e.y, e.z, v.x, v.y, v.z);
        if (overlaps_abc(e, v, cell))
          overlap_v_index = k;
      }
      if (overlap_v_index == -1) {
        printf("ERROR: could not construct net: no vertex could be found which "
        ...
        if (verbose)
          printf("DEBUG: vertex %d edge %d at %.3f %.3f %.3f overlaps with "
                 "vertex %d at %.3f %.3f %.3f with periodicity (%d %d %d)\n",
  ...
  if (num_connections % 2 != 0) {
    printf("ERROR: there are an odd number (%d) of one-way connections between "
    ...
    if (!considered.at(i)) { // we are looking at one-way connections - if we
      ...
      for (int j = 0; j < num_connections;
        ...
        if (!considered.at(j)) {
          ...
          if (matches(ci, cj)) {
            if (matched) {
              printf("ERROR: determined that vertex %d edge %d overlaps with "
            ...
            cij.e2 = cj.e1;
            if (verbose)
              printf("DEBUG: vertex %d edge %d overlaps with vertex %d edge %d "
      ...
      if (!matched) {
        printf("ERROR: could not find corresponding connection for vertex %d "
               "edge %d overlapping with vertex %d\n",
               ci.v1, ci.e1, ci.v2);
        exit(EXIT_FAILURE);
      }
    }
...
vector<MOLECULE> get_multiple_best_RMSD_fits(MOLECULE mol, ATOM_NETWORK *cell,
  ...
  if (num_fit_sites_dummy != num_mol_sites_dummy ||
      num_fit_sites_no_dummy !=
          num_mol_sites_no_dummy) { // we can only fit the molecule to the given
                                    // vertex is the number of sites is the same
                                    // in each
    printf("ERROR: cannot fit molecule with %d sites and %d dummy sites to a "
      ...
      if (vector_of_fits.at(i).rmsd <
        ...
        if (molecule_alignment_chemistry_is_unique(
          ...
          if (verbose) {
            ...
            FILE *rotmol = fopen(rotmol_name.c_str(), "w");
            if (rotmol == NULL) {
              printf("ERROR: could not open output rotated molecule file with "
                     "name %s\n",
                     rotmol_name.c_str());
              exit(EXIT_FAILURE);
            }
            MOLECULE rotated =
                translate(f.fit, get_mol_site_CoM(&f.fit).scale(-1));
            write_molecule(
                rotmol, &rotated, net, vertex_ID, -1,
                true); // oriented to basic vertex basic_ID with symmetry "-1"
                       // (not applicable here) - this will be noted in the file
...
ATOM_NETWORK build_framework(vector<MOLECULE> *molecules, ATOM_NETWORK *cell,
  ...
  if (num_mols != num_vertices) {
    printf("ERROR: the number of oriented molecules (%d) is not equal to the "
...
double determine_edge_length(vector<MOLECULE> *molecules, ATOM_NETWORK *cell,
  ...
  for (int i = 0; i < num_mols; i++)
    assembled_molecules->push_back(molecules->at(
        i)); // this vector starts out the same as the molecules vector, but the
             // molecules are then each shifted into their relative positions to
             // form the assembled_molecules
  while (num_placed < 2 && placed_ok) {
    if (verbose)
      printf("DEBUG: num_placed = %d; placing...\n", num_placed);
    ...
    if (verbose)
      printf("DEBUG: after calling place_molecule, %d molecules have been "
             "placed\n",
             new_num_placed);
  ...
  if (!placed_ok || num_placed != 2) {
    ...
    //    valid?\n"); exit(EXIT_FAILURE);
    if (verbose)
      printf("NOTICE: could not place any molecules - returning invalid "
             "distance\n");
    return -1;
  }
...
ATOM_NETWORK connect_molecules(vector<MOLECULE> *molecules, ATOM_NETWORK *cell,
  ...
  double sum_edge_lengths = 0;
  if (verbose)
    printf("DEBUG: about to place molecules\n");
  ...
  if (num_placed != num_mols) {
    ...
    if (verbose)
      printf("NOTICE: only %d of %d molecules were fixed in place during "
  ...
  if (verbose) {
    ...
    for (int i = 0; i < num_loops; i++) {
      ...
      printf("DEBUG: unique loop was found with periodicity (%d %d %d) and "
             "Cartesian shift %.3f %.3f %.3f\n",
  ...
  if (found_all_uc_vectors) {
    if (verbose)
      printf("DEBUG: found all uc vectors from loop analysis\n");
  } else {
  ...
  if (dimensionality == 2) {
    if (verbose)
      printf("DEBUG: imposing a three dimensional structure on this "
    ...
    if (num_override != 1) {
      printf("ERROR: was expecting to override exactly 1 cell side length "
...
bool check_for_collision(ATOM_NETWORK *framework,
  ...
  bool verbose = false;
  if (verbose)
    printf("DEBUG: checking for collisions...\n");
      ...
      if (assembled_molecules->at(i).atoms_type.at(j) != "H") { // if not H
        ...
        if (is_site_j) {
          local_site_abc_vector.push_back(abc_j);
                                      ...
                                      // collision detection near to dummy sites
        } else {
  ...
  for (int i = 0; i < num_total_atoms && !collision; i++) {
    for (int j = i + 1; j < num_total_atoms && !collision; j++) {
      if (atom_molecule_id_vector.at(i) ==
        ...
        if (!bonded_xyz(assembled_molecules->at(atom_molecule_id_vector.at(i))
          ...
          if (bonded_abc(atom_abc_vector.at(i), atom_abc_vector.at(j),
                         framework)) {
            if (verbose)
              printf("NOTICE: non-bonded atoms in building block %d are bonded "
          ...
          for (int da = -1; da < 2 && num_periodic_bonds < 2; da++) {
            for (int db = -1; db < 2 && num_periodic_bonds < 2; db++) {
              for (int dc = -1; dc < 2 && num_periodic_bonds < 2; dc++) {
                ...
                if (bonded_xyz(image_j_xyz, centre_xyz))
                  num_periodic_bonds++;
              }
            }
          }
          if (num_periodic_bonds > 1) {
            if (verbose)
              printf("NOTICE: more than one (%d) bonds were detected between "
        ...
        if (!atom_is_close_to_site_vector.at(i) ||
          ...
          if (verbose)
            printf("NOTICE: a bond was detected between atoms that are not "
                   "both part of a site: %s (fractional %.3f/%.3f/%.3f) and %s "
          ...
          if (!mols_connected) {
            if (verbose)
              printf("NOTICE: a bond was detected between disconnected "
                     "building blocks %d and %d, constituting a collision\n",
                     atom_molecule_id_vector.at(i),
                     atom_molecule_id_vector.at(j));
            collision = true;
          }
        }
      }
    ...
    if (num_tokens == 0) {
      ...
      anticipating_node = false;
               ...
               token.at(0) == "EDGE") { // should only occur when the different
                                        // kind of atom (node) is used - handled
                                        // differently to edges above
      if (num_tokens > 6) { // read from this line if there are fields, else
                            // read from next line...
      ...
      if (verbose)
        printf(" --- successfully finished parsing net file --- \n");
      read_ok = true;
    }
    if (uses_node && uses_atom) {
      printf("NET ERROR: detected that both atom and node flags are used in "
             "the input net file - this is currently assumed to indicate an "
             "invalid input file\n");
      exit(EXIT_FAILURE);
    }
  }
...
void parse_atom(vector<string> *token, int first_index, ATOM_NETWORK *cell,
  ...
  for (int i = 0; i < v.expected_edges; i++) {
    if (fgets(line, maxline, cgd) != NULL) {
      ...
      if (local_token.at(0) == "edge" || local_token.at(0) == "EDGE") {
        ...
        printf("NET ERROR: atom with %d edges was declared but the string "
               "\"%s\" was read instead of the data for edge ID %d\n",
               v.expected_edges, local_token.at(0).c_str(), i);
        exit(EXIT_FAILURE);
      }
    } else {
      printf("NET WARNING: %d edges were expected but file ended after reading "
             "%d edges\n",
  ...
  if (v.expected_edges == 2) { // i.e., if 2c
    if (fgets(line, maxline, cgd) != NULL) {
      ...
      if (local_token.at(0) == "dummy_edge" ||
        ...
        printf("NET ERROR: dummy edge for 2c atom with index %d was declared "
               "but the string \"%s\" was read instead of the data for the "
               "dummy edge\n",
               (*atom_index), local_token.at(0).c_str());
        exit(EXIT_FAILURE);
      }
    } else {
      printf("NET WARNING: dummy edge was expected for 2c atom with index %d, "
             "but file ended instead\n",
             (*atom_index));
      // exit(EXIT_FAILURE);
    }
  }
  ...
  cell->vertex_symmetry_operators.push_back(
      0); // the vertices read from file are the basic ones, before any symmetry
          // operator (or you could say after operator 0, which does nothing)
  ...
  v.expected_edges = convertToInt(
      token->at(first_index +
                1)); // this many edges are expected, but note that with this
                     // file format, the edges only come once all nodes are read
                     // and, in all likelihood, are not given exhaustively
  cell->vertices.push_back(v);
  cell->vertex_symmetry_operators.push_back(
      0); // the vertices read from file are the basic ones, before any symmetry
          // operator (or you could say after operator 0, which does nothing)
...
void parse_group(vector<string> *token, int first_index, ATOM_NETWORK *cell) {
  ...
  if (verbose)
    printf("parsed group name %s\n", sym_name.c_str());
  int sym_ID = get_sym_ID(sym_name);
  if (verbose)
    printf("\ti.e. group ID %d\n", sym_ID);
  cell->sym_ID = sym_ID;
...
void parse_edge(vector<string> *token, int first_index, ATOM_NETWORK *cell,
  ...
  bool verbose = false;
  if (!uses_node) {
    printf("NET ERROR: parsed unexpected \"edge\" field, when edges where "
  ...
  if (v_index == -1) {
    printf("NET ERROR: could not match this edge start position to a "
           "previously parsed vertex\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (v_index == -1) {
    ...
    */
    if (verbose)
      printf("DEBUG: could not match this edge end position to a previously "
...
void add_missing_edges(
  ...
  for (int i = 0; i < num_vertices; i++) {
    ...
    if (verbose)
      printf("\n");
    if (verbose)
      printf("DEBUG: vertex ID %d at %.3f %.3f %.3f [%.3f %.3f %.3f] has %d "
    ...
    if (num_edges < num_expect_edges) {
      if (verbose)
        printf("DEBUG: BASIC VERTEX ID %d has fewer edges than expected - "
               "filling in missing edges by symmetry operations\n",
               i);
      if (verbose)
          ...
          if (overlaps_xyz(edge_xyz, new_edges_xyz.at(k)))
            this_read_edge_is_dupe =
                true; // we should not reject edges because their end points
                      // overlap periodically - think pcu! they are valid unless
                      // they overlap in xyz!
        }
        ...
        for (int k = 0; k < num_sym_images; k++) {
          XYZ this_base_sym_edge = trans_to_origuc(
              equiv.at(k)); // the kth symmetry image of the jth edge that was
                            ...
                            // periodic image, and add them to the new vector if
                            // they are not a duplicate of one already added
          ...
          for (int im = 0; im < num_periodic_images; im++) {
            ...
            if (length_ok) {
              ...
              if (verbose)
                printf("\t\tDEBUG: edge at %.3f %.3f %.3f [%.3f %.3f %.3f] "
                       "length is ok - checking for overlap against the "
                       "existing %d edges...\n",
              ...
              for (int l = 0; l < num_new_edges && !this_sym_edge_is_dupe;
                ...
                if (verbose)
                  printf("\t\t\tDEBUG: checking against existing edge %d at "
                         "%.3f %.3f %.3f [%.3f %.3f %.3f]...\n",
                ...
                if (overlaps_xyz(sym_edge_xyz, new_edges_xyz.at(l)))
                  this_sym_edge_is_dupe =
                      true; // we should not reject edges because their end
                            // points overlap periodically - think pcu! they are
              ...
              if (!this_sym_edge_is_dupe) { // the kth symmetry image of the jth
                ...
                if (verbose)
                  printf("\t\t\tDEBUG: this sym edge is unique\n");
              } else {
                if (verbose)
      ...
      int num_new_edges = new_edges_abc.size();
      if (verbose)
        printf("\tDEBUG: after examining symmetry, the following %d edges were "
               "identified:\n",
               num_new_edges);
      ...
      if (num_new_edges == num_expect_edges) {
        if (verbose)
          printf("DEBUG: the correct number of edges, %d, have been identified "
        ...
        if (num_orphan_edges_check != num_orphan_edges) {
          printf("ERROR: attempting to interpret orphan edges in order to "
                 "complete the basic net, but the number of start points (%d) "
                 "is not equal to the number of end points (%d)\n",
            ...
            if (overlaps_abc(this_sym_orphan_vertex, v.abc,
              ...
              vector<XYZ> equiv_orphan_edges = GetEquivalentPositions(
                  ...
                  &orphan_edge); // get all the symmetry positions of this
                                 // orphan edge, but we only need to look at the
                                 // particular symmetry operation in question,
                                 // hopefully
              XYZ this_sym_orphan_edge = equiv_orphan_edges.at(k);
              if (verbose)
                printf("\tDEBUG: the corresponding edge end symmetry image is "
                       "at %.3f %.3f %.3f\n",
              ...
              for (int l = 0; l < num_read_edge_lengths && !length_ok; l++) {
                if (fabs(sym_orphan_edge_length - read_edge_lengths.at(l)) <
                  ...
                  length_ok = true;
                  if (verbose)
                    printf("\t\tDEBUG: length is ok - %.3f is similar to "
                           "desired %.3f\n",
        ...
        num_new_edges = new_edges_abc.size();
        if (verbose)
          printf("DEBUG: after examining symmetry AND orphan edges, the "
                 "following %d edges were identified:\n",
                 num_new_edges);
        for (int j = 0; j < num_new_edges; j++) {
          XYZ this_new_edge_abc = new_edges_abc.at(j);
          XYZ this_new_edge_xyz = new_edges_xyz.at(j);
        ...
        if (num_new_edges == num_expect_edges) {
          ...
          printf("ERROR: %d edges were expected but after examining edges by "
      ...
      if (verbose)
        printf("DEBUG: the correct number of edges, %d, have been identified - "
               "no action required for this vertex\n",
               num_edges);
    } else {
      printf("ERROR: more edges than expected - error case in file parsing, "
             "probably a bug!\n");
      exit(EXIT_FAILURE);
    }
  }
...
void add_2c_vertices_and_normal_edges(
  ...
  for (int i = 0; i < num_candidates; i++) {
    ...
    for (int j = 0; j < i && keep; j++) {
      if (overlaps_abc(candidates.at(i), candidates.at(j), cell))
        keep = false;
    }
...
void add_2c_dummy_edges(
  ...
  bool verbose = false;
  if (verbose)
    printf("DEBUG: adding dummy edges to 2-c vertices to represent orthogonal "
  ...
  for (int i = 0; i < num_full_v; i++) {
    ...
    if (num_e ==
      ...
      for (int ex = 0; ex < num_examined_already && !examined_already; ex++) {
        if (basic_IDs_examined.at(ex) == basic_ID)
          examined_already = true;
      }
      if (!examined_already) { // only need to find the direction if this vertex
                               // is not a symmetry image of another
        ...
        if (sym_op != 0) { // double check that the symmetry operator is zero
          ...
          printf("ERROR: found a 2-c vertex ID %d based on underlying 2-c "
                 "vertex ID %d, which has not already been examined, but the "
                 "symmetry operator is non-zero (%d)\n",
                 i, basic_ID, sym_op);
          exit(EXIT_FAILURE);
        }
        ...
        if (v1 == -1 ||
          ...
          printf("ERROR: could not identify the two full cell vertices "
                 "connected to 2-c vertex %d\n",
                 i);
          exit(EXIT_FAILURE);
        }
        XYZ v1_xyz =
            ...
            if (acute_angle_between_degrees >
              ...
              XYZ projection_1_vector_norm =
                  get_vector_from_to(other_edge_1_end_xyz,
                      ...
                      .unit();
              if (verbose)
                printf("DEBUG: projection vector is from %.3f %.3f %.3f and "
                       "norm is %.3f %.3f %.3f\n",
                       other_edge_1_projection_xyz.x,
                       other_edge_1_projection_xyz.y,
                       other_edge_1_projection_xyz.z,
                       projection_1_vector_norm.x, projection_1_vector_norm.y,
                       projection_1_vector_norm.z);
              for (int k = 0; k < num_edges_v2 && !non_parallel_direction_found;
                   k++) {
                if (k != e2) { // don't select the edge that connects to the 2-c
                               // vertex in question ...
                  ...
                  double acute_angle_between_degrees =
                      min(angle_between_degrees, 180.0 - angle_between_degrees);
                  if (verbose)
                    printf("DEBUG: considering edge id %d of v2 which points "
                           "to position %.3f %.3f %.3f (%.3f degrees angular "
                           "difference)\n",
                  ...
                  if (acute_angle_between_degrees >
                    ...
                    if (verbose)
                      printf("DEBUG: new_v_xyz (image of 'basic' 2c vertex) = "
                             "%.3f %.3f %.3f\n",
                    ...
                    XYZ projection_2_vector_norm =
                        get_vector_from_to(other_edge_2_end_xyz,
                            ...
                            .unit();
                    if (verbose)
                      printf("DEBUG: projection vector is from %.3f %.3f %.3f "
                             "and norm is %.3f %.3f %.3f\n",
                    ...
                    if (acute_angle_between_degrees >
                      ...
                      if (verbose)
                        printf("DEBUG: non-parallel combo found: %.3f %.3f "
                               "%.3f and %.3f %.3f %.3f leading to %.3f %.3f "
                               "%.3f - with edge length of %.3f and a vector "
                               "position %.3f %.3f %.3f, the edge ends at %.3f "
                               "%.3f %.3f (in abc, %.3f %.3f %.3f)\n",
        ...
        if (dummy_edge_abc.x > 1000 && dummy_edge_abc.y > 1000 &&
          ...
          printf("ERROR: could not find a candidate orthogonal direction with "
        ...
        */
        full_cell->vertices.at(i).dummy_edges.push_back(equiv_dummy_e.at(
            sym_op)); // this is nice - we know that we are working on symmetric
...
void read_xyz(FILE *input, MOLECULE *mol, const char *filename) {
  ...
  for (int i = 0; i < num_atoms; i++) {
    ...
    if (fgets(ch, length, input) != NULL) {
      ...
      printf("ERROR: could not read expected atom coord string from %s - %d "
             "out of %d atom coords were read\n",
    ...
    for (int i = 0; i < name_length && digit_pos == -1; i++) {
      if (isdigit(name_string[i]))
        digit_pos = i;
    }
    if (digit_pos == 0) { // first character was a digit - this is no good
      printf("ERROR: could not parse label from atom beginning with a digit in "
             "read_xyz: %s: %s\n",
