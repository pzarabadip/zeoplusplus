...
void PORE::findChannelsAndPockets(DIJKSTRA_NETWORK *dnet,
  ...
  for (unsigned int i = 0; i < dnet->nodes.size(); i++) {
    if (dnet->nodes.at(i).active == true)
      noAccDijkNodes++;
...
void CHANNEL::findChannels(DIJKSTRA_NETWORK *dnet, vector<bool> *infoStorage,
  ...
  for (unsigned int i = 0; i < pores.size(); i++) {
    if (pores[i].dimensionality > 0)
      channels->push_back(CHANNEL(&pores[i]));
  };
  pores.clear();
}
...
pair<double, pair<double, double>> CHANNEL::findFreeIncludedSphereDiameter() {
  ...
  for (int i = 0; i < nodes.size(); i++) {
    ...
    if (i == 0) {
      ...
      if (didfdif.first > maxdidfdif.first)
        maxdidfdif.first = didfdif.first;
    };
  };
...
CHANNEL::findFreeIncludedSphereDiameterforNode(
  ...
  maxdi = lnodes.at(nodeIndex).max_radius; // (it is not really the max but will
                                           // be analyzed to get max)
...
void PORE::getRestrictingDiameters(int nSegments, vector<int> vorNetID,
  ...
  vector<int> PoreNodesSegmentIDs; // will store segment ID for each node in the
                                   // current PORE
