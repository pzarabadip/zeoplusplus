...
#define LARGE_RATIO                                                            \
  1.3 // ratio between radii with triggers high accuract setting (replacement of
      // an atom with a large cluster)
  ...
  for (unsigned int i = 0; i < atmnet->atoms.size(); i++) {
    if (i == 0) {
      ...
      if (atmnet->atoms.at(i).radius < minr)
        minr = atmnet->atoms.at(i).radius;
      if (atmnet->atoms.at(i).radius > maxr)
        maxr = atmnet->atoms.at(i).radius;
    };
  };
