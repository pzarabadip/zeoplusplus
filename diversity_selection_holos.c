...
main(int argc, char *argv[]) {
  ...
  if (argc != needed_args && argc != needed_args + optional_args) {
    printf("Number of command line arguments should be %d. Please try again, "
           "e.g.:\n",
           needed_args);
    printf("./a.out list_of_all_holos_files num_structures "
           "diversity_output_filename num_desired max_similarity "
           "DIFFUSION_RANGE SIMILARITY_MEASURE\n\t(optional args: run_index "
           "num_runs, for distributed seeding)\n");
    printf("Program will identify diverse structures until either num_desired "
           "are chosen, or max_similarity is exceeded; give either criterion a "
  ...
  printf("Program will identify diverse structures until either %d are chosen, "
         "or %.6f similarity is exceeded (if either value is <0, this "
  ...
  if (argc == needed_args + optional_args) {
    ...
    if (num_runs <= 0) {
      printf("ERROR: num_runs should be positive, but has value %d. Please try "
             "again.\n",
             num_runs);
      exit(EXIT_FAILURE);
    }
    if (run_index >= num_runs || run_index < 0) {
      printf("ERROR: run_index should be in range 0 to num_runs-1, i.e. 0 to "
             "%d, but has value %d. Please try again.\n",
  ...
  for (i = 0; i < num_structures; i++) {
    ...
    if (status == -1) {
      printf("ERROR: could not read file name for structure index %d. Please "
             "try again.\n",
             i);
      exit(EXIT_FAILURE);
    }
    ...
    if (hologram_array[i].num_entries > 0)
      is_valid[i] = 1;
    fclose(holo);
  }
  fclose(in);
  ...
  while ((num_selected < num_desired || num_desired < 0) &&
    ...
    for (j = 0; j < num_structures; j++) {
      ...
      if ((max_sim < current_lowest_max_sim || current_lowest_max_sim < 0) &&
        ...
        candidates.push_back(t);
                 ...
                 max_sim >= 0) { // tie-break scenario, where at least one
    ...
    if (current_lowest_max_sim > max_similarity && !(max_similarity < 0)) {
      ...
      printf("Not proceeding because max_similarity has been exceeded "
             "(%.6f>%.6f)\n",
      ...
      while (num_candidates > 1) {
        ...
        if (DIFFUSION_RANGE + num_tiebreaks > NUM_BINS / 2) {
          ...
          num_candidates = 1;
          printf("%d structures all have max similarity of %.6f to any "
                 "selected structure; force-ending tiebreak because increasing "
                 "diffusion will have no effect\n",
                 num_candidates, current_lowest_max_sim);
        } else {
          printf("%d structures all have max similarity of %.6f to any "
                 "selected structure; tiebreak based on increasing diffusion "
                 "to %d and recalculating similarities:\n",
                 num_candidates, current_lowest_max_sim,
  ...
  if (num_desired != num_selected && exceeded_max_similarity == 0) {
    printf("ERROR: finished loop without exceeding %.6f similarity, having "
