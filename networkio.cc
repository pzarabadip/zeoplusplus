    ...
    exception: // I needed an easy way to jump out of the _loop command if an
      ...
      if (token.size() > 0) {
        ...
        if (token[0].substr(0, 5).compare(list[0]) == 0) { // name of unit cell
          ...
          if (!tokenized) {
            printf("\n#####\n##### WARNING: parsed a loop in cif file, but "
                   "data was not present\n#####\n\n");
          } else {
            // printf("DEBUG: exited loop successfully, having read data line
            // \"%s\"\n", line.c_str()); collecting the data associated with each
            // column and put
            // in correct place
  ...
  if (input == NULL) {
    ...
    while (found_final == 0) {
      if (fgets(this_line, 500, input) != NULL) {
        ...
        printf("ERROR: finished parsing ARC file before finding geometry "
               "section\n");
        //        exit(EXIT_FAILURE);
        fclose(input);
        return false;
    ...
    while (found_atoms == 0) {
      if (fgets(this_line, 500, input) != NULL) {
        ...
        printf("ERROR: finished parsing ARC file before finding individual "
               "atom information\n");
        //        exit(EXIT_FAILURE);
        fclose(input);
        return false;
  ...
  if (input.is_open() == true) {
    ...
    while (!end) {
      ...
      if (str1.compare("end") == 0 || str1.compare("END") == 0) {
        ...
        if (!CAR_USE_ATOM_TYPE_OVER_NAME)
          newAtom.type = str1;
